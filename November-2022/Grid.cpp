...
void bfs(pi s, vector<pi> graph[n][m], vvi &visited, vvi &distance) {
    visited[s.first][s.second] = 1;
    queue<pi> q;
    q.push(s);
    while(!q.empty()) {
        pi u = q.front(); q.pop();
        int ud = distance[u.first][u.second]+1;
        for(auto v : graph[u.first][u.second]) {
            if(!visited[v.first][v.second]) {
                distance[v.first][v.second] = ud;
...
void solve() {
    ...
    vvi visited(n,vi(m,0)); vvi distance(n,vi(m,0));
    ...
    bfs(mp(0,0), graph, visited, distance);
