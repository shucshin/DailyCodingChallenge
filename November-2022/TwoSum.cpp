...
public:
    vector<int> twoSum(vector<int>& nums, int target) {
        ...
        for(int i = 0; i < org.size(); i++) {
            if(org[i] == a) {ans.push_back(i); continue;}
            if(org[i] == b) {ans.push_back(i); continue;}
        }
        return ans;
    }
};
...
void solve() {
    vi nums1 = {2,7,11,15}, nums2 = {3,2,4}, nums3 = {3,3}, nums4 = {-1,-2,-3,-4,-5};
    Solution sol;
    printVec(sol.twoSum(nums1,9));
    printVec(sol.twoSum(nums2,6));
    printVec(sol.twoSum(nums3,6));
    printVec(sol.twoSum(nums4,-8));
}
