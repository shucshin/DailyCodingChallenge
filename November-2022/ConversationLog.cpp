...
bool comparePair(pair<string, int> a, pair<string, int> b) {return a.second == b.second ? a.first < b.first : a.second > b.second;} //if(a.f == b.f) return a.s < b.s; return a.f < b.f;}
...
void solve() {
    ...
    rep(i,0,m) {
        ...
        bool fst = true;
        while(ss >> word) {
            if(fst) {s = word; fst = false;}
            else {ans[s].insert(word); freq[word]++;}
        }
    }
    ...
    for(auto f : freq) {
        ...
        bool everyone = true;
        for(auto a : ans) {
            everyone &= a.second.count(w);
        }
        if(everyone) vec.pb(mp(w,f.second));
    }

    sort(all(vec), comparePair);
    if(vec.size() == 0) cout << "ALL CLEAR\n"; 
    rep(i,0,vec.size()) {
