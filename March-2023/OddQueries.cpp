...
void solve() {
    int n, q, a; cin >> n >> q;
    vll v(n); rep(i,0,n) {cin >> a; a%2==0 ? v[i]=0 : v[i]=1;}
    vll st(n*4); buildST(v,st,1,0,n-1);

    //cout << "Total Sum: " << rsqST(st,1,0,n-1,0,n-1) << endl;

    int l, r, k;
    rep(i,0,q) {
        cin >> l >> r >> k;
        l--; r--;
        //cout << "Left: " << rsqST(st,1,0,n-1,0,l-1) << " Right: " << rsqST(st,1,0,n-1,r+1,n-1) << endl;
        ll sum = rsqST(st,1,0,n-1,0,l-1) + rsqST(st,1,0,n-1,r+1,n-1);
        //cout << "Sum: " << sum << endl;
        k%2==0 ? sum+=0 : sum+=(r-l+1);
