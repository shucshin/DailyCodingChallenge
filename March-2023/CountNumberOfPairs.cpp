...
int solve(int k, string s) {
    ...
    for(auto a : m) {
        ...
        if(!m[l] && !m[u]) continue; // both 0 so nothing to do
        else if(m[l] && m[u]) { // example a=2,A=4
            ans += min(m[l], m[u]); // can make 2 pairs
            available += abs(m[l]-m[u])/2; // available += (2/2), can make 1 pair with AA
            m[l] = 0; m[u] = 0; // a=0, A=0
        }
        else {// one of them is 0 so add possible pairs
            available += max(m[l], m[u])/2;
            m[l] = 0; m[u] = 0;
        }
    }
    ans += min(available, k); // can do k until it reaches the available ones to change case
    return ans;
}
